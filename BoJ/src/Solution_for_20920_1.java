...
public class Solution_for_20920_1 {
    public static void main(String[] args) throws IOException {
        ...
        TreeMap<Map.Entry<String,Integer>,Integer> vocaNote = new TreeMap<>(new VocaComparator());

        for (int i = 0; i < N; i++) {
            String voca = br.readLine();
            if (voca.length() >= M) {
                if (vocaNote.containsKey(voca)) {
                    vocaNote.put(Map.Entry<voca, vocaNote.get(voca) + 1> , vocaNote.get(voca) + 1);
                }
                else
                    vocaNote.put(Map.Entry<voca,1> , 1);
            }
        }

        System.out.println(vocaNote);
